...
|-----------|----------|----------|
| Architecture & design | Martin | Action structure, API strategy, label schema, enterprise vs org scope |
| GitHub Actions implementation | Whistler | action.yml, TypeScript code, Octokit API calls, label upsert logic, pagination |
| Testing & validation | Mother | Unit tests, edge cases, label idempotency, mocking Octokit |
| Code review | Martin | Review PRs, check quality, suggest improvements |
| Testing | Mother | Write tests, find edge cases, verify fixes |
| Scope & priorities | Martin | What to build next, trade-offs, decisions |
